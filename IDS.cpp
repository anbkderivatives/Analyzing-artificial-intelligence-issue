////////////////////////////////////		ITERATIVE-DEEPENING SEARCH ( I D S )		//////////////////////////////////////////////////////
//		Σε αυτόν τον αλγόριθμο ακολουθούνται περιγραφικός οι εξής ενέργειες:
//	-Εκτελούνται οι παρακάτω ενέργεις όσο το όριο της επαναληπτικής εκβάθυνσης αυξάνεται και δεν έχει βρεθεί λύση στο πρόβλημα 
...
srand(time(NULL));
	...
	Stoiva stoivaSol; //temp αντικείμενο που χρησιμεύει για να πάρει τιμές του αντικειμένου stoiva, όπως θα δούμε παρακάτω
	int n; //πλήθος ατόμων
...
////////////////////////////	STARTS ITERATIVE-DEEPENING SEARCH ( I D S )	////////////////////////////////
int counter=0; //μετρητής επαναληπτικής εκβάθυνσης
while(counter>=0)
{
...
while(stoiva.head !=NULL && stoiva.head->nodekomvos->Depth <= counter)	 
{
...
////--------------	ΕΞΕΤΑΣΗ ΤΟΥ ΤΡΕΧΟΝΤΑ ΚΟΜΒΟΥ	--------------------------------
	...
	if(NodeCheck==true)		
	{
		//Ektypwsi Komvwn pou eksetazontai----------------------------
...
//	-Εισάγουμε στην Ουρα τον αντίστοιχο κόμβο
  ...
  if(stoiva.head->nodekomvos->Depth < counter) //έλεγχος ορίου (check Level limit)
  {
...
//------------------	ΤΕΛΟΣ ΕΠΕΚΤΑΣΗΣ		---------------------------
     }//end if έλεγχος ορίου (check Level limit)
   }//end if (NodeCheck)
  

  }//end while(stoiva.head != NULL && stoiva.head->nodekomvos->Depth <= counter) 
  /*
  cout<<"Depth: "<<stoiva.head->nodekomvos->Depth<<endl;
  cout<<"Counter: "<<counter<<endl;
  cout<<"foundsolution: "<<foundsolution<<endl;system("pause");
  */
if(foundsolution==true && stoiva.head !=NULL) {stoivaSol=stoiva; break;}
counter++;
}//end while(counter>0)

Stoiva stoiva=stoivaSol; //αντιγραφή τιμών
...
SolutionNodes solNodes; 
	...
	cout<<"\n\nMia lysi pou dinei o IDS einai:\n\n";
	...
	cout<<"\nTo Plithos twn Komvwn opou episkefthike o IDS, einai :"<<PlithosKomvwn<<endl;
